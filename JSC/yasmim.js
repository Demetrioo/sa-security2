...
let userExcluir = document.getElementById("excluir")
...
function Logar(){
    ...
    if (logou == 1){
          ...
          window.location.href="login-conta.html"
...
function Excluir (){
    nomes = JSON.parce(localStorage.getItem("userLogado"))
    ...
    let posicaoExcluir
     ...
     for(i=0; i < nomes.length; i++){
       ...
       if(userExcluir.value == nomes[i]){
         ...
         // Joga para o LocalStorage novamente
         localStorage.setItem("userLogado", JSON.stringify(nomes))
